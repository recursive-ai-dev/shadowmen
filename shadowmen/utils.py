        ...
        with _wm_lock:
            if _wm_available is None:
                ...
                if not _wm_available:
                    ...
                    log.info(
                        "Wayland session detected — wmctrl functionality may be limited."
                    )
...
def _get_windows_win32(sw: int, sh: int) -> list[WindowSnapshot]:
    ...
    try:
        ...
        for w in gw.getAllWindows():
            if not w.visible or w.isMinimized or not w.title:
                continue
            if w.width < _MIN_WINDOW_SIZE or w.height < _MIN_WINDOW_SIZE:
                continue
            if (
                w.width >= sw - _FULLSCREEN_MARGIN
                and w.height >= sh - _FULLSCREEN_MARGIN
            ):
                continue
            ...
            wins.append(
                WindowSnapshot(w.left, w.top, w.width, w.height, str(w._hWnd), biome)
            )
        return wins
    except ImportError:
        return []
    ...
    try:
        ...
        for w in window_list:
            ...
            if width < _MIN_WINDOW_SIZE or height < _MIN_WINDOW_SIZE:
                continue
            if width >= sw - _FULLSCREEN_MARGIN and height >= sh - _FULLSCREEN_MARGIN:
                continue

            wid = str(w.get("kCGWindowNumber", "0"))
            title = w.get("kCGWindowName", "").lower()
            biome = "neutral"
            if "terminal" in title or "iterm" in title:
                biome = "hardened"
            elif "browser" in title or "safari" in title or "chrome" in title:
                biome = "information-rich"

            wins.append(
                WindowSnapshot(int(x), int(y), int(width), int(height), wid, biome)
            )
        return wins
    except ImportError:
        return []
